...
QImage WithSmallCounter(QImage image, CounterLayerArgs &&args) {
	...
	const auto d = Dimensions{
		...
		.font = int(base::SafeRound(args.size.value() / 2.6)),
		.delta = int(base::SafeRound(args.size.value() / ((textSize < 2) ? 6.4 : 16.))),
		.radius = int(base::SafeRound(args.size.value() / ((textSize < 2) ? 4. : 4.5))),
	};
